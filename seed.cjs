...
const STUDENTS = [
  ...
  { name: "Aliza",     email: "Aliza@uni.edu",    rollNo: "CS-202", password: "1234" },
...
async function seed() {
  ...
  try {
    ...
    for (const s of STUDENTS) {
      ...
      await mongoose.connection.db.collection('students').insertOne({ ...s, password: hashed, createdAt: new Date() });
